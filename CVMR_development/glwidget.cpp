    ...
    : QOpenGLWidget(parent)
{
    ...
    m_bundleAdjustmentPoints.clear();
}
...
void GLWidget::paintGL() {
    ...
    projection.perspective(45.0f, float(width())/height(), 0.01f, 20000.0f);

    // View matrix (user-controlled)
    QMatrix4x4 view;
    // Move camera up and back for a better grid view
    view.translate(-m_viewX, -m_viewY, -m_viewZ);
    ...
    // --- Render loaded 3D point cloud if present ---
    if (!m_loadedPointCloud.empty()) {
        m_shader.setUniformValue("model", QMatrix4x4());
        m_shader.setUniformValue("color", QVector3D(1.0f, 0.0f, 1.0f));
        m_shader.setUniformValue("useTexture", 0); // <--- Ensure color mode

        float pointSize = .50f; // <--- Try a huge value for debug
        ...
        glBufferData(GL_ARRAY_BUFFER, m_loadedPointCloud.size() * sizeof(QVector3D), m_loadedPointCloud.data(), GL_STATIC_DRAW);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
        glEnableVertexAttribArray(0);
        //fix texture
        glDisableVertexAttribArray(1); // <--- Disable unused texcoord
        glDrawArrays(GL_POINTS, 0, m_loadedPointCloud.size());
        glBindVertexArray(0);
        ...
        if (err != GL_NO_ERROR) qDebug() << "OpenGL error after point cloud draw:" << err;
    }

    // --- Render bundle adjustment 3D points if present ---
    if (!m_bundleAdjustmentPoints.empty()) {
        m_shader.setUniformValue("model", QMatrix4x4());
        m_shader.setUniformValue("color", QVector3D(0.0f, 1.0f, 0.0f)); // Green for BA points
        m_shader.setUniformValue("useTexture", 0);

        float pointSize = 2.0f; // Larger points for BA results
        glPointSize(pointSize);
        glEnable(GL_PROGRAM_POINT_SIZE);
        GLuint baVbo, baVao;
        glGenVertexArrays(1, &baVao);
        glGenBuffers(1, &baVbo);
        glBindVertexArray(baVao);
        glBindBuffer(GL_ARRAY_BUFFER, baVbo);
        glBufferData(GL_ARRAY_BUFFER, m_bundleAdjustmentPoints.size() * sizeof(QVector3D), m_bundleAdjustmentPoints.data(), GL_STATIC_DRAW);
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
        glEnableVertexAttribArray(0);
        glDisableVertexAttribArray(1);
        glDrawArrays(GL_POINTS, 0, m_bundleAdjustmentPoints.size());
        glBindVertexArray(0);
        glDeleteBuffers(1, &baVbo);
        glDeleteVertexArrays(1, &baVao);
        glPointSize(1.0f);

        GLenum err = glGetError();
        if (err != GL_NO_ERROR) qDebug() << "OpenGL error after BA point cloud draw:" << err;
    }
...
void GLWidget::setCameraExtrinsics(const std::vector<QMatrix4x4>& extrinsics) {
    ...
    float nearZ = 0.25f, farZ = 1.0f;  //Change here to make Camera frustums more visible
...
void GLWidget::toggleRays() {
    ...
    update();   // Ensure immediate repaint. "good" idea
}
...
void GLWidget::keyPressEvent(QKeyEvent *event) {
    ...
    switch (event->key()) {
        ...
        case Qt::Key_Y:   m_viewY += step; break;
        case Qt::Key_H: m_viewY -= step; break;
        case Qt::Key_Plus:
...
QMatrix4x4 GLWidget::getExtrinsicMatrixForCamera(int camIdx) const {
    if (camIdx >= 0 && camIdx < int(m_cameraExtrinsics.size())) {
        return m_cameraExtrinsics[camIdx];
    }
    return QMatrix4x4();
}
...
void GLWidget::loadSIFTDataset(const QString& dirPath) {
    ...
    // Match images and KRTs by filename (without extension)
    ...
    // --- Update intrinsics to match first camera's K ---
    m_intrinsic = intrinsics[0];
    emit intrinsicMatrixChanged();

    // --- Load extrinsics into GUI and update all camera state ---
    setCameraExtrinsics(extrinsics);
    emit extrinsicMatrixChanged();

    // --- Optionally, update images as well ---
    //setCameraImages(imagePaths);

    // --- Run SIFT and triangulate ---
    runSIFTAndTriangulate(imagePaths, extrinsics, poseNames);

    // --- Write results to separate files ---
    writeSIFT3DResultsToFiles(dirPath + "/sift3d");
}

void GLWidget::writeSIFT3DResultsToFiles(const QString& dirPath) {
    QDir outDir(dirPath);
    if (!outDir.exists()) outDir.mkpath(".");
    for (const auto& kv : m_siftResults) {
        QString fname = outDir.filePath(kv.first + ".sift3d.txt");
        QFile file(fname);
        if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) continue;
        QTextStream out(&file);
        const auto& res = kv.second;
        int count = 0;
        for (const auto& pt : res.points3d) {
            out << pt.x << " " << pt.y << " " << pt.z << "\n";
            ++count;
        }
        file.close();
        if (count == 0) {
            qWarning() << "writeSIFT3DResultsToFiles: No 3D points written for" << fname;
        }
    }
}

// Slot to load a 3D point cloud file and store for rendering
void GLWidget::loadSIFTPointCloudFile(const QString& filePath) {
    m_loadedPointCloud.clear();
    QFile file(filePath);
    ...
    while (!in.atEnd()) {
        ...
        if (ok1 && ok2 && ok3) {
            m_loadedPointCloud.push_back(QVector3D(x, y, z));
        }
    }
    file.close();

    // --- Debug: print min/max/center of point cloud ---
    if (!m_loadedPointCloud.empty()) {
        QVector3D minPt = m_loadedPointCloud[0], maxPt = m_loadedPointCloud[0];
        for (const auto& pt : m_loadedPointCloud) {
        ...
        QVector3D center = (minPt + maxPt) * 0.5f;
        qDebug() << "Loaded point cloud:" << m_loadedPointCloud.size() << "points";
        qDebug() << "  min:" << minPt << "max:" << maxPt << "center:" << center;
    } else {
        qDebug() << "Loaded point cloud: 0 points!";
    }
    ///centerViewOnPointCloud(); bad,stupid
    //update();  STupid, bad
 
    update();
}

// SIFT feature extraction and triangulation
...
void GLWidget::centerViewOnPointCloud() {
    if (m_loadedPointCloud.empty()) return;
    QVector3D minPt = m_loadedPointCloud[0], maxPt = m_loadedPointCloud[0];
    for (const auto& pt : m_loadedPointCloud) {
        minPt.setX(std::min(minPt.x(), pt.x()));
        minPt.setY(std::min(minPt.y(), pt.y()));
...
// Bundle Adjustment NPZ loading function
void GLWidget::loadBundleAdjustmentNPZ(const QString& npzPath) {
    qDebug() << "Loading bundle adjustment NPZ from:" << npzPath;
    
    // For now, we'll implement a simple NPZ reader
    // In practice, you might want to use Python to extract the data or a C++ NPZ library
    
    // Since NPZ format is complex, let's create a simple text-based loader
    // that reads the extracted data in a simple format
    
    // First, let's try to read it as if it was already extracted to text files
    QFileInfo npzInfo(npzPath);
    QString basePath = npzInfo.absolutePath();
    QString baseName = npzInfo.baseName();
    
    // Look for extracted text files
    QString pointsFile = basePath + "/" + baseName + "_points.txt";
    QString camerasFile = basePath + "/" + baseName + "_cameras.txt";
    
    qDebug() << "Looking for points file:" << pointsFile;
    qDebug() << "Looking for cameras file:" << camerasFile;
    
    // Load 3D points
    m_bundleAdjustmentPoints.clear();
    QFile pFile(pointsFile);
    if (pFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            if (parts.size() >= 3) {
                ...
                float z = parts[2].toFloat(&ok3);
                if (ok1 && ok2 && ok3) {
                    m_bundleAdjustmentPoints.push_back(QVector3D(x, y, z));
                }
            }
        }
        pFile.close();
        qDebug() << "Loaded" << m_bundleAdjustmentPoints.size() << "bundle adjustment points";
    } else {
        qDebug() << "Could not open points file:" << pointsFile;
        
        // Fallback: try to read the NPZ path directly as a simple point cloud
        QFile directFile(npzPath);
        if (directFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
            QTextStream in(&directFile);
            while (!in.atEnd()) {
                QString line = in.readLine().trimmed();
            ...
            directFile.close();
            qDebug() << "Loaded" << m_bundleAdjustmentPoints.size() << "bundle adjustment points from direct file";
        }
    }
    
    // Load camera poses
    std::vector<QMatrix4x4> bundleAdjustmentCameras;
    QFile cFile(camerasFile);
    if (cFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        while (!in.atEnd()) {
            ...
            // Try to read 4x4 matrix (16 values)
            QStringList parts = line.split(QRegularExpression("\\s+"));
            if (parts.size() >= 16) {
                QMatrix4x4 mat;
                bool allOk = true;
                ...
                if (allOk) {
                    bundleAdjustmentCameras.push_back(mat);
                }
            }
        }
        cFile.close();
        qDebug() << "Loaded" << bundleAdjustmentCameras.size() << "bundle adjustment cameras";
        
        // Set the loaded cameras as the current camera extrinsics
        if (!bundleAdjustmentCameras.empty()) {
            setCameraExtrinsics(bundleAdjustmentCameras);
        }
    } else {
        qDebug() << "Could not open cameras file:" << camerasFile;
    }
    
    // Center view on bundle adjustment points if they exist
    if (!m_bundleAdjustmentPoints.empty()) {
        QVector3D minPt = m_bundleAdjustmentPoints[0], maxPt = m_bundleAdjustmentPoints[0];
        for (const auto& pt : m_bundleAdjustmentPoints) {
            minPt.setX(std::min(minPt.x(), pt.x()));
            minPt.setY(std::min(minPt.y(), pt.y()));
            minPt.setZ(std::min(minPt.z(), pt.z()));
            maxPt.setX(std::max(maxPt.x(), pt.x()));
        ...
        QVector3D center = (minPt + maxPt) * 0.5f;
        QVector3D size = maxPt - minPt;
        float maxDim = std::max({size.x(), size.y(), size.z()});
        
        qDebug() << "Bundle adjustment points - min:" << minPt << "max:" << maxPt << "center:" << center;
        
        m_viewX = -center.x();
        m_viewY = -center.y();
        m_viewZ = -center.z() - maxDim; // Move back based on size
        
        update();
    }
}

void GLWidget::clearBundleAdjustmentData() {
    m_bundleAdjustmentPoints.clear();
    update();
}