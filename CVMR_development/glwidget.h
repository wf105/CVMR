...
public:
    ...
    QMatrix4x4 getExtrinsicMatrixForCamera(int camIdx) const; 
    int getCameraCount() const;
    bool isAllCamerasEditing() const;
    void setAllCamerasEditing(bool enabled);
    void toggleRays(); // Already implemented in .cpp

    // Bundle Adjustment NPZ loading
    void loadBundleAdjustmentNPZ(const QString& npzPath);
    void clearBundleAdjustmentData();
...
public slots:
    ...
    void loadSIFTPointCloudFile(const QString& filePath); // NEW

    void centerViewOnPointCloud(); //debugging
...
private:
    ...
    // Bundle adjustment and SIFT point clouds
    std::vector<QVector3D> m_bundleAdjustmentPoints; // 3D points from bundle adjustment
    std::vector<QVector3D> m_loadedPointCloud; // SIFT point cloud
    ...
    void writeSIFT3DResultsToFiles(const QString& dirPath); // NEW
