...
euler_orders = ['ZYX', 'ZXY', 'YXZ', 'YZX', 'XYZ', 'XZY']
...
def process_with_permutations(input_txt, output_base):
    ...
    for (
        ...
        for i, row in df.iterrows():
            ...
            R = rotation_matrix(h, p, r, order=euler_order)
            R = chirality_fn(R)
        ...
        mat_array = np.array(matrices).flatten()
        mat_hash = hashlib.sha256(mat_array.tobytes()).hexdigest()

        if mat_hash in seen_hashes:
            total_skipped += 1
            print(f"X Skipping duplicate: {variant_id}")
            continue

        seen_hashes.add(mat_hash)
        total_written += 1
        print(f"Y Saving unique: {variant_id}")

        with open(output_file, 'w') as f:
            for mat in matrices:
                mat_str = ' '.join(f'{v:.8f}' for v in mat)
                f.write(f"{mat_str}\n")
    ...
    print(f"Unique files written: {total_written}")
    print(f"Duplicates skipped: {total_skipped}")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Generate unique extrinsics across Euler permutations.")
